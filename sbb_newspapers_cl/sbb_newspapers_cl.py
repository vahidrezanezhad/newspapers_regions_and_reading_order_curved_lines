...
class sbb_newspapers_cl:
    def __init__(self, image_dir, f_name,dir_out, dir_models):
        ...
        self.model_region_dir_p_low_scale = dir_models +'/model_main_scale_1_1und_1_2_corona_great.h5'
    ...
    def get_image_and_scales(self):
        ...
        self.low_scale_metric=self.image.shape[0]
        self.image_org=np.copy(self.image)
        self.height_org = self.image.shape[0]
        self.width_org = self.image.shape[1]
        
        
        if self.image.shape[0] < 1700:
    ...
    def extract_page(self):
        ...
        gc.collect()
        return croped_page, page_coord,box
    ...
    def seperate_lines_new(self, img_path, thetha,num_col):
        ...
        if num_col==1:
            num_patches=int(img_path.shape[1]/250.)
        else:
            num_patches=int(img_path.shape[1]/250.)
    ...
    def seperate_lines_new_inside_teils(self, img_patch, thetha):
    ...
    def do_work_of_textline_seperation(self,queue_of_all_params,polygons_per_process, index_polygons_per_process,textline_mask_tot,mask_texts_only,num_col,scale_par):
        ...
        for iiii in range(len(polygons_per_process)):
            ...
            textline_rotated_seperated=self.seperate_lines_new(textline_biggest_region[y:y+h,x:x+w],0,num_col+1)
        ...
        queue_of_all_params.put([index_polygons_per_process_per_process,textregions_cnt_tot_per_process, textlines_cnt_tot_per_process ])
    ...
    def get_regions_from_xy_low_scale(self,img):
        img_org=np.copy(img)
    ...
    def run(self):
        ...
        if self.low_scale_metric<1700:
            text_regions_p_1=self.get_regions_from_xy_low_scale(self.image)
        else:
        ...
        image_page,page_coord,box=self.extract_page()
        
        text_regions_p_1, _ = self.crop_image_inside_box(box, text_regions_p_1)
        ...
        #print(slope_first,'slope')
        image_page_rotated,textline_mask_tot,text_regions_p_1=self.rotation_not_90_func(image_page,textline_mask_tot,text_regions_p_1,slope_first)
        
        ##self.get_layout(image_page_rotated)
        
        #plt.imshow(text_regions_p_1)
        #plt.show()
        
        #print(np.unique(text_regions_p_1))
        ...
        # resize image regions and cv2.res to bigger scale
        if img_revised_tab.shape[0]<4000:
            scale_param=1.4
        elif img_revised_tab.shape[0]>=4000 and img_revised_tab.shape[0]<5000:
            scale_param=1.2
        elif img_revised_tab.shape[0]>=5000 and img_revised_tab.shape[0]<6000:
            scale_param=1.1
        else:
            scale_param=1
        ...
        img_revised_tab= self.resize_image(img_revised_tab, int(img_revised_tab.shape[0]*scale_param), int(img_revised_tab.shape[1]*scale_param) )
        ##print(text_regions_p.shape,np.unique(text_regions_p))
        
        #plt.imshow(text_regions_p)
        #plt.show()
        
        textline_mask_tot= self.resize_image(textline_mask_tot, int(textline_mask_tot.shape[0]*scale_param), int(textline_mask_tot.shape[1]*scale_param) )
        #print(textline_mask_tot.shape,np.unique(textline_mask_tot))
        
        #plt.imshow(textline_mask_tot)
        #plt.show()
        
        textline_mask_tot=cv2.erode(textline_mask_tot,kernel=self.kernel,iterations=1)
        ...
        #mask_texts_only=(text_regions_p[:,:]==1)*1
        ...
        index_polygons=np.array(range(len(contours_only_text_parent)))
        processes = []
        nh=np.linspace(0, len(contours_only_text_parent), num_cores+1)
        
        
        for i in range(num_cores):
            polygons_per_process=contours_only_text_parent[int(nh[i]):int(nh[i+1])]
            index_polygons_per_process=index_polygons[int(nh[i]):int(nh[i+1])]
                                                                          
            processes.append(Process(target=self.do_work_of_textline_seperation, args=(queue_of_all_params,  polygons_per_process, index_polygons_per_process,textline_mask_tot,text_only,num_col,scale_param)))
            #print(i,'h1111111111111111111111')
        ...
        for i in range(num_cores):
            ...
            indexes_for_sub_process=list_all_par[0]
            textregions_cnt_tot_for_sub_process=list_all_par[1]
            textlines_cnt_tot_for_sub_process=list_all_par[2]
            
            for j in range(len(textregions_cnt_tot_for_sub_process)):
                indexes_tot.append(indexes_for_sub_process[j])
                textregions_cnt_tot.append(np.array(textregions_cnt_tot_for_sub_process[j],dtype=np.int32))
                textlines_cnt_tot.append(textlines_cnt_tot_for_sub_process[j] )
        ...
        #print(textregions_cnt_tot,textregions_cnt_tot[0])
        

        cx_text_only,cy_text_only ,x_min_text_only, _, _ ,_=self.find_new_features_of_contoures(textregions_cnt_tot)
        ...
        for iij in range(len(boxes)):
            ...
            for i in range(len(args_contours_box)):
                ...
                con_inter_box.append( textregions_cnt_tot[args_contours_box[i] ]  )
        ...
        for i in range(0):#(len(order_of_texts_tot)):
            ...
            img_con=cv2.fillPoly(img_con, pts =[textregions_cnt_tot[order_of_texts_tot[i] ] ] , color=(255,255,255))

        
        
        order_of_texts_tot=[]
        #id_of_texts_tot=[]
        for tj1 in range(len(textregions_cnt_tot)):
        ...
        self.write_into_page_xml(textregions_cnt_tot,page_coord,self.dir_out , order_text_new , id_of_texts_tot,textlines_cnt_tot,
...
def main(image,out, model):
    ...
    x = sbb_newspapers_cl(image, None, out, model)
    x.run()
